...
class PageBackground {
	...
	private initBackground = () => {
		...
		this.baseCtx.font = "28px monospace";
		this.baseCtx.textAlign = "start";
		this.baseCtx.textBaseline = "top";
		const isLight = document.documentElement.getAttribute("data-theme") === "light";
		this.baseCtx.fillStyle = isLight ? "rgba(0, 0, 0, 0.03)" : "rgba(255, 255, 255, 0.01)";
		...
		this.overlayCtx.font = "bold 28px monospace";
		this.overlayCtx.textAlign = "start";
		this.overlayCtx.textBaseline = "top";
		this.overlayCtx.fillStyle = this.primaryColor;
		this.overlayCtx.shadowBlur = 16;
		this.overlayCtx.shadowColor = this.primaryColor;
		this.overlayCtx.globalAlpha = 0;
	...
	public resizeBackground = () => {
		...
		// Update primary color on resize/theme change
		this.primaryColor = window
			.getComputedStyle(document.documentElement)
			.getPropertyValue("--accent")
			.trim() || "#a855f7";
...
let backgroundInstance: PageBackground | null = null;
...
document.addEventListener("astro:page-load", () => {
	...
	backgroundInstance = new PageBackground(canvas, overlayCanvas);

	if (resizeHandler) {
		window.removeEventListener("resize", resizeHandler);
	}
	resizeHandler = () => backgroundInstance?.resizeBackground();
	...
	themeObserver = new MutationObserver((mutations) => {
		for (const mutation of mutations) {
			if (mutation.attributeName === "data-theme") {
				backgroundInstance?.resizeBackground();
			}
		}
	});
